...
interface SummarizedNode {
  ...
  meta: {
    ...
    style?: Partial<
      Pick<
        FigmaNode,
        'fills' | 'strokes' | 'strokeWeight' | 'strokeAlign' | 'cornerRadius' | 'effects' | 'blendMode' | 'opacity'
      >
    >
    text?: Partial<
      Pick<
        FigmaNode,
        'characters' | 'style' | 'lineHeightPercent' | 'letterSpacing' | 'textAlignHorizontal' | 'textAlignVertical'
      >
    >
    semantic?: Partial<Pick<FigmaNode, 'componentId' | 'componentProperties' | 'isMask' | 'layoutGrids'>>
...
const newApp = createApp().route('/frames', GetFrame)
serve({
  fetch: newApp.fetch,
  port,
})
