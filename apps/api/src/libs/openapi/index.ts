import type { ApiResponseContent, requestOptions } from './type'

import type { paths } from '../../__generated__/schema'
import type { ApiPathParameters, ApiQueryParameters, ApiRequestBody, OptionalizeNeverParameters } from './type'
import type { ApiPath } from './type'
import type { MethodKeys } from './type'
 ...
 */
export const requestFigma = async <T extends ApiPath, M extends MethodKeys<paths[T]>>(
  path: T,
  method: M,
...
): Promise<Response & { json: () => Promise<ApiResponseContent<paths[T], M>> }> => {
  ...
  if (
    ...
    for (const p of Object.entries(parameters.body)) {
      ...
      if (p[1] || typeof p[1] === 'boolean' || typeof p[1] === 'number' || typeof p[1] === 'string') {
        body[p[0]] = p[1]
      }
    }
  }
